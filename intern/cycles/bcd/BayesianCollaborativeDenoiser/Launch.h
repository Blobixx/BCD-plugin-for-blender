...
// #include "Utils.h
// #include "device/bcd/BayesianCollaborativeDenoiser/Denoiser.h"
// #include "device/bcd/BayesianCollaborativeDenoiser/MultiscaleDenoiser.h"
// #include "device/bcd/BayesianCollaborativeDenoiser/IDenoiser.h"

// #include "device/bcd/BayesianCollaborativeDenoiser/SpikeRemovalFilter.h"

// #ifdef FOUND_CUDA
// #include "device/bcd/BayesianCollaborativeDenoiser/CudaHistogramDistance.h"
// #endif
...
namespace bcd
{
	...
	bool parseProgramArguments(ProgramArguments& o_rProgramArguments, const char* m_denoisedOutputFilePath,
			const char* m_colorImage, const char* m_histogramImage, const char* m_covarianceImage,
			float m_histogramPatchDistanceThreshold,
			int m_patchRadius, int m_searchWindowRadius,
			float m_minEigenValue,
			bool m_useRandomPixelOrder,
			bool m_prefilterSpikes,
			float m_prefilterThresholdStDevFactor,
		...
		o_rProgramArguments.m_denoisedOutputFilePath = string(m_denoisedOutputFilePath);
		ofstream outputFile(o_rProgramArguments.m_denoisedOutputFilePath, ofstream::out | ofstream::app);
		if(!outputFile){
			cout << "ERROR in program arguments: cannot write output file '" << o_rProgramArguments.m_denoisedOutputFilePath << "'" << endl;
			return false;
		...
		if (!ImageIO::loadEXR(o_rProgramArguments.m_colorImage, m_colorImage)){
			cout << "ERROR in program arguments: couldn't load input color image file '" << m_colorImage << "'" << endl;
			return false;
		}else{
			missingColor = false;
		}
		...
		Deepimf histAndNbOfSamplesImage;
		if (!ImageIO::loadMultiChannelsEXR(histAndNbOfSamplesImage, m_histogramImage)){
			cout << "ERROR in program arguments: couldn't load input histogram image file '" << m_histogramImage << "'" << endl;
			return false;
		}else{
			Utils::separateNbOfSamplesFromHistogram(o_rProgramArguments.m_histogramImage, o_rProgramArguments.m_nbOfSamplesImage, histAndNbOfSamplesImage);
			missingHist = false;
		...
		if(!ImageIO::loadMultiChannelsEXR(o_rProgramArguments.m_covarianceImage, m_covarianceImage)){
			cout << "ERROR in program arguments: couldn't load input covariance matrix image file '" << m_covarianceImage << "'" << endl;
			return false;
		}else{
			missingCov = false;
		}

		o_rProgramArguments.m_histogramPatchDistanceThreshold = m_histogramPatchDistanceThreshold;
		o_rProgramArguments.m_patchRadius = m_patchRadius;
		o_rProgramArguments.m_searchWindowRadius = m_searchWindowRadius;
		o_rProgramArguments.m_minEigenValue = m_minEigenValue;
		o_rProgramArguments.m_useRandomPixelOrder = m_useRandomPixelOrder;
		o_rProgramArguments.m_prefilterSpikes = m_prefilterSpikes;
		o_rProgramArguments.m_prefilterThresholdStDevFactor = m_prefilterThresholdStDevFactor;
		o_rProgramArguments.m_markedPixelsSkippingProbability = m_markedPixelsSkippingProbability;
		o_rProgramArguments.m_nbOfScales = m_nbOfScales;
		o_rProgramArguments.m_nbOfCores = m_nbOfCores;
		o_rProgramArguments.m_useCuda = m_useCuda;
			...
			bool m_useCuda = true)
	{
		...
		if(!parseProgramArguments(programArgs, m_denoisedOutputFilePath,
			...
			m_prefilterSpikes,
			m_prefilterThresholdStDevFactor,
			m_markedPixelsSkippingProbability,
			m_nbOfScales,
			m_nbOfCores,
			m_useCuda)){
